...
public class GameMapUI extends JLabel implements Observer {	
	...
	/**
	 * This method is called when the Model changes its data.
	 * Hence it should repaint the screen of this View.
	 */
	...
	public BufferedImage getImageMap(BufferedImage bi) {
		...
		for (int i=0; i<15; i++) {
			for (int j =0; j<15; j++) {
				g.drawImage(map.getRm().getAtom( intMap[i][j] ), j*30, i*30, c);
			}
		}
		return bi;
	}
